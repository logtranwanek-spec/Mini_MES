...
app.MapGet("/api/tracking/journey", async (string date, AppDbContext db) =>
{
    try
    {
        ...
        // TẠO MỚI MoProgress NẾU CHƯA CÓ (CHỈ DÙNG MO)
        try
        {
            var allSteps = result
                .SelectMany(r => r.Steps.Select(s => new { Mx = r.Mx, Step = s }))
                .ToList();

            var existingMoKeys = (await db.MoProgresses
                .Select(p => p.MO.ToUpper())
                .ToListAsync())
                .ToHashSet();
...
public class As400ScanPollingService : BackgroundService
{
    ...
    private async Task PollOnceAsync(IServiceScope scope, CancellationToken token)
    {
        ...
        var _lastScanTime = lastScanTimeSetting != null
        ...
        foreach (var group in updatedMoGroups)
        {
            var mp = await db.MoProgresses
                .FirstOrDefaultAsync(m => m.MO == group.Key, token);

            if (mp != null)
            {
                foreach (var row in group)
                {
                    var logExists = await db.ScanLogs
                        .AnyAsync(l => l.MO == row.MO && l.ScanTime == row.ScanTime, token);
                    if (!logExists)
                    {
                ...
                mp.ActualQty = await db.ScanLogs
                    .Where(s => s.MO == group.Key)
                    .SumAsync(s => s.Qty, token);

                mp.LastScanTime = await db.ScanLogs
                    .Where(s => s.MO == group.Key)
                    .MaxAsync(s => (DateTime?)s.ScanTime, token);
